class Forecast < ApplicationRecord
  ...
  def self.by_time!(timestamp)
    ...
    prev_forecast = Forecast.where("epoch_time <=?", Time.at(timestamp.to_i)).sorted.first
    next_forecast = Forecast.where("epoch_time >=?", Time.at(timestamp.to_i)).order(:epoch_time).first

    closest_forecast = [prev_forecast, next_forecast]
                         .compact
